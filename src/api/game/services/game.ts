...
async function getGameInfo(slug) {
    try {
        ...
        const short_description = raw_description.textContent.slice(0, 200);
        ...
        return {
            description,
            short_description,
            rating: ratingElement ? ratingElement.getAttribute("xlink:href").replace("#", "").replace(/_/g, "") : "BR0",
        };
    } catch (error) {
...
async function setImage({ image, game, field = "cover" }) {
    ...
    formData.append("files", buffer, { filename: `${game.name}.jpg` });
